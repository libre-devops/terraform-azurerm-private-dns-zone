resource "azurerm_private_dns_zone" "private_dns_zone" {
  for_each            = var.create_default_privatelink_zones ? [] : toset(["true"])
...
resource "azurerm_private_dns_zone" "privatelink_dns_zones" {
  for_each            = var.create_default_privatelink_zones == true ? { for idx, value in var.privatelink_dns_zones : value.zone_name => merge(value, { index = idx }) } : {}
  name                = each.key
  resource_group_name = try(var.rg_name, null)
  tags                = try(var.tags, null)
...
resource "azurerm_private_dns_zone_virtual_network_link" "private_dns_zone_link" {
  for_each              = try(var.link_to_vnet, true) == true && var.create_default_privatelink_zones ? [] : toset(["true"])
  name                  = var.vnet_link_name == null ? "pdnsz-${replace(azurerm_private_dns_zone.private_dns_zone[each.key].name, ".", "")}" : try(var.vnet_link_name, null)
  resource_group_name   = try(var.rg_name, null)
...
resource "azurerm_private_dns_zone_virtual_network_link" "privatelink_dns_zone_link" {
  for_each              = try(var.link_to_vnet, true) == true && var.create_default_privatelink_zones == true ? { for idx, value in var.privatelink_dns_zones : value.zone_name => merge(value, { index = idx }) } : {}
  name                  = var.vnet_link_name == null ? "pdnsz-${replace(azurerm_private_dns_zone.privatelink_dns_zones[each.key].name, ".", "")}" : try(var.vnet_link_name, null)
  resource_group_name   = try(var.rg_name, null)
